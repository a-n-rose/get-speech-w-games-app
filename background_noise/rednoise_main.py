...
from rednoise_fun import wave2stft, stft2power, stft2amp, get_mean_bandwidths, rednoise, stft2wave, savewave
...
if __name__ == '__main__':
    ...
    #get mean power (for each bandwidth)
...
#    stft_red_100 = rednoise_short(noise_pw_mean,sp_amp[100],speech_stft[100],start_col,col_len) 
    stft_red = [rednoise(noise_amp_mean,sp_amp[row],speech_stft[row]) for row in range(speech_stft.shape[0])]
    
    #transform stft back to wave
    speech_red = stft2wave(stft_red,speech_sr)
    savewave("rednoise1.wav",speech_red,speech_sr)
